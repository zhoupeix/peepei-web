      ...
      <div class="ui-contact-us">
        ...
        <div class="ui-relate">
          ...
          <div class="ui-relate-box">
            <ul>
              <li v-for="(item,index) in moreNews" :key="item._id"> 
                <router-link
              class="router-link"
              :to="{
                path: `/news/${item._id}`,
                query: {pagination:index === 7?pagination=2:pagination=1,index:index===7?index=1:index }
              }"
              id="next"
              :title="item.title"
...
export default {
  ...
  async asyncData({ $axios, params, query }) {
    const news = await $axios.get('res/articles',{
      params:{
        query:'',
        pageNum:1,
        pageSize:10
    ...
    const res = await $axios.get(`res/articles/${params.id}`)
    const data = await $axios.get('res/articles', {
      params: {
        //模糊搜索参数
        query: '',
        //当前第几页
        
        pageNum: query.index === 6 ? query.pagination + 1 : query.pagination,
        //显示几条
        pageSize: 7
      }
    })
    const preData = await $axios.get('res/articles', {
      params: {
        //当前第几页
        query: '',
        pageNum:
          query.index === 0 && query.pagination != 1
            ? query.pagination - 1
            : query.pagination,
        //显示几条
        pageSize: 7
    ...
    let nextPagination
    let preIndex
    if (data.data.countNum > (query.pagination - 1) * 7 + query.index + 1) {
      if (query.index === 6) {
        nextIndex = 0
        nextPagination = query.pagination + 1
      } else {
        nextIndex = query.index + 1
        nextPagination = query.pagination
      }
      nextNews = data.data.items[nextIndex]
    }
    if (!(query.pagination === 1 && query.index === 0)) {
      if (query.index === 6) {
        preIndex = 5
        prePagination = query.pagination
      } else if (query.index === 0) {
        preIndex = 6
        prePagination = query.pagination - 1
      } else {
        preIndex = query.index - 1
        prePagination = query.pagination
      }
      console.log('prepagition', prePagination)
      preNews = preData.data.items[preIndex]
    }

    console.log('parmas', query)
    console.log('nextNews', nextNews)
    console.log('preNews', preNews)
    console.log('moreNews',news.data)
    return {
