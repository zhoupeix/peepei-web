      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <el-input placeholder="请输入客户名称" v-model="queryInfo.query" clearable @clear="fetch">
        ...
        <el-col :span="6">
          <div class="grid-content bg-purple">
          <el-button type='primary' @click="$router.push('/custom/create')" class="el-icon-plus">添加客户</el-button>
          </div>
          </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    handleClick(row) {
      this.$router.push(`/custom/edit/${row._id}`);
    },
    async remove(row) {
      this.$confirm(`操作将永久删除 “${row.name}” , 是否继续?`, "提示", {
        ...
        const res = await this.$http.delete(`rest/customs/${row._id}`);
        this.$message({
          type: "success",
          message: "删除成功!"
        });
    ...
    async fetch() {
      const res = await this.$http.get("rest/customs",{
